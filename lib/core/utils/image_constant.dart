class ImageConstant {
  // Image folder path
  static String imagePath = 'assets/images';

  // Logo page images
  static String imgImage1 = '$imagePath/img_image_1.png';

  // Intro Page images
  static String imgImage1444x360 = '$imagePath/img_image_1_444x360.png';

  // LOGIN  Page images

  static String signUpScreenLogo = '$imagePath/signup_screen_logo.png';

  static String signUpScreenLogo2 = '$imagePath/signup_screen_logo_2.png';

  static String imgImage9 = '$imagePath/img_image_9.png';

  static String imgVector281x266 = '$imagePath/img_vector_281x266.png';

  //Create Post Images

  static String imgOrientationLan = '$imagePath/img_orientation_lan.svg';

  static String imgShare = '$imagePath/img_share.svg';

  // Credentials Page 2  images
  static String imgUser = '$imagePath/img_user.svg';

  static String imgIc24Check1 = '$imagePath/img_ic24_check_1.svg';

  static String imgGroup = '$imagePath/img_group.svg';

  static String imgGroupGray900 = '$imagePath/img_group_gray_900.svg';

  static String imgGroup18 = '$imagePath/img_group_18.svg';

  // Settings page images
  static String imgRectangle4191 = '$imagePath/img_rectangle_4191.png';

  static String imgRectangle4190 = '$imagePath/img_rectangle_4190.png';

  static String imgThumbsUp = '$imagePath/img_thumbs_up.svg';

  // Savings images
  static String imgChildIcon = '$imagePath/img_child_icon.svg';

  static String imgClose = '$imagePath/img_close.svg';

  static String imgCoffee = '$imagePath/img_coffee.svg';

  static String imgTicket1Hobby = '$imagePath/img_ticket_1_hobby.svg';

  // Socials Page images
  static String imgChatBubbleOva = '$imagePath/img_chat_bubble_ova.svg';

  // Home Page images
  static String imgUserSingleNeu = '$imagePath/img_user_single_neu.svg';

  // Joe's Profile images
  static String imgPolygon1 = '$imagePath/img_polygon_1.png';

  static String imgBase = '$imagePath/img_base.png';

  static String imgInner = '$imagePath/img_inner.png';

  static String imgHighlight = '$imagePath/img_highlight.png';

  static String imgMaskGroup = '$imagePath/img_mask_group.png';

  static String imgMaskGroupWhiteA700 =
      '$imagePath/img_mask_group_white_a700.png';

  static String imgInner42x48 = '$imagePath/img_inner_42x48.png';

  static String imgHighlight51x56 = '$imagePath/img_highlight_51x56.png';

  static String imgMaskGroupBlueGray100 =
      '$imagePath/img_mask_group_blue_gray_100.png';

  static String imgBase69x63 = '$imagePath/img_base_69x63.png';

  static String imgInner45x42 = '$imagePath/img_inner_45x42.png';

  static String imgHighlights = '$imagePath/img_highlights.png';

  static String imgMaskGroupBlueGray10045x41 =
      '$imagePath/img_mask_group_blue_gray_100_45x41.png';

  static String imgImage6 = '$imagePath/img_image_6.png';

  // Emergency page images
  static String imgSettingsDeepPurple900 =
      '$imagePath/img_settings_deep_purple_900.svg';

  // SOS activation images
  static String imgVectorRed300 = '$imagePath/img_vector_red_300.png';

  static String imgImage11 = '$imagePath/img_image_11.png';

  // Breathing exercises images
  static String imgImage12 = '$imagePath/img_image_12.png';

  // Breathing exercises - with GIF images
  static String imgBreathingGif = '$imagePath/img_breathing_gif.png';

  // Discover - List View images
  static String imgArrowRight = '$imagePath/img_arrow_right.svg';

  // Discover - Individual Detailed View - Rehab Centers images
  static String imgCheckupMedical = '$imagePath/img_checkup_medical.svg';

  // Discover - Indvidual Detailed View - Recovery Programs images
  static String imgGroup4568 = '$imagePath/img_group_4568.svg';

  // Common images
  static String imgRight = '$imagePath/img_right.svg';

  static String imgDesignWizardB = '$imagePath/img_design_wizard_b.png';

  static String imgDesignWizardBOnerror = '$imagePath/img_design_wizard_b_onerror.png';

  static String imgDesignWizardBadges3d = '$imagePath/img_design_wizard_badges_3d.png';

  static String imgGroup553 = '$imagePath/img_group_553.png';

  static String imgEmergency = '$imagePath/img_emergency.svg';

  static String imgRightOnprimarycontainer =
      '$imagePath/img_right_onprimarycontainer.svg';

  static String imgUserGray30001 = '$imagePath/img_user_gray_300_01.svg';

  static String imgUserDeepOrange100 =
      '$imagePath/img_user_deep_orange_100.svg';

  static String imgUserOnerror = '$imagePath/img_user_onerror.svg';

  static String imgUserOnprimary = '$imagePath/img_user_onprimary.svg';

  static String imgUserOnerrorcontainer =
      '$imagePath/img_user_onerrorcontainer.svg';

  static String imgGrid = '$imagePath/img_grid.svg';

  static String imgGridOrange90001 = '$imagePath/img_grid_orange_900_01.svg';

  static String imgNavEducation = '$imagePath/img_nav_education.svg';

  static String imgGroupOrange90001 = '$imagePath/img_group_orange_900_01.svg';

  static String imgSettings = '$imagePath/img_settings.svg';

  static String imgLocationPin3 = '$imagePath/img_location_pin_3.svg';

  static String imgRightOnprimarycontainer23x32 =
      '$imagePath/img_right_onprimarycontainer_23x32.svg';

  static String imgImage7 = '$imagePath/img_image_7.png';

  static String imgHeartRewardS = '$imagePath/img_heart_reward_s.svg';

  static String imgImage3 = '$imagePath/img_image_3.png';

  static String imgHeartRewardSOrange90001 =
      '$imagePath/img_heart_reward_s_orange_900_01.svg';

  static String imgMagnifyingglassglasssearchmagnifying =
      '$imagePath/img_magnifyingglassglasssearchmagnifying.svg';

  static String imgArrowLeft = '$imagePath/img_arrow_left.svg';

  static String imgImage13 = '$imagePath/img_image_13.png';

  static String imgShareLinkShareTransmit =
      '$imagePath/img_share_link_share_transmit.svg';

  static String imgLinkedin = '$imagePath/img_linkedin.svg';

  static String imgPhoneAndroid = '$imagePath/img_phone_android.svg';

  static String imgPhoneButton = '$imagePath/img_phone_button.svg';

  static String rehabCenterImage = '$imagePath/rehab_center_image1.png';

  static String imgInstagram = '$imagePath/img_instagram.svg';

  static String imgFacebook = '$imagePath/img_facebook.svg';

    static String mquitServiceImg = '$imagePath/mquit_service.png';

  static String imgTwitterMediaTwitterSocial =
      '$imagePath/img_twitter_media_twitter_social.svg';

  static String imageNotFound = 'assets/images/image_not_found.png';
}
